...
import type { OutboxNotifyTransport } from './transport/outbox-transport.port.js';
import { PgNotifyTransport } from './transport/pg-notify.transport.js';

/**
 * Trace family provider. Mirrors the request-context triplet from the
 * product (getRequestContext / runWithTraceContext); the framework adapter
 * binds it to its async-local context. Absent a provider, enqueue fails
 * closed with OutboxRequestContextMissingError (same invariant as the
 * product).
 */
...
export interface OutboxServiceOptions {
  getTraceContext?: () => OutboxTraceContext | undefined;
  transport?: OutboxNotifyTransport;
  logger?: OutboxLogger;
}
...
export class OutboxService {
  ...
  private readonly getTraceContext: () => OutboxTraceContext | undefined;
  private readonly transport: OutboxNotifyTransport;
  ...
  constructor(
    ...
    this.getTraceContext = options.getTraceContext ?? (() => undefined);
    this.transport = options.transport ?? new PgNotifyTransport();
  }
  ...
  async notifyTopic(manager: EntityManager, topic: string): Promise<void> {
    await this.transport.notify(manager, topic);
  }
}
