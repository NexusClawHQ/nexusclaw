...
import {
  ...
  applyJsonRedactionPointers,
} from './redaction.js';
import { ToolCallRecord } from './entities/tool-call-record.entity.js';
import { ToolCallStatus } from './tool-call-status.js';

/**
 * Structural stand-ins for the product's executable-assets contracts
 * (nexusclaw-core shared/agent-executable-assets, private). Cover exactly the
 * surface this service consumes; assignable from the product's concrete types.
 */
export interface AgentCodeToolExportV1 {
  readonly redaction?: {
    readonly logPolicy?: 'metadata_only' | string;
    readonly inputJsonPointers?: readonly string[];
    readonly outputJsonPointers?: readonly string[];
  };
}
...
export class ToolCallLifecycleService {
  constructor(private readonly records: Repository<ToolCallRecord>) {}
